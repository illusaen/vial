...
bool process_select_word(uint16_t keycode, keyrecord_t* record, uint16_t sel_keycode) {
    ...
    if (keycode == sel_keycode && record->event.pressed) { // On key press.
        ...
        if (!shifted) { // Select word.
            ...
            register_code(reversed ? KC_LEFT : KC_RGHT);
            state = STATE_WORD;
        } else { // Select line.
            if (state == STATE_NONE) {
                if (is_operating_system_mac()) {
                ...
                register_code(reversed ? KC_UP : KC_DOWN);
                state = STATE_LINE;
            }
        }
        return false;
    ...
    switch (state) {
        case STATE_WORD:
            unregister_code(reversed ? KC_LEFT : KC_RGHT);
            if (is_operating_system_mac()) {
        ...
        case STATE_LINE:
            unregister_code(reversed ? KC_UP : KC_DOWN);
            state = STATE_SELECTED;
            break;
